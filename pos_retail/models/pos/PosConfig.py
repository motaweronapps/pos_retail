...
class PosConfig(models.Model):
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        if context.get('action_pos_config_kanban', None):
            domain = ['|', ('user_id', '=', self.env.user.id), ('id', 'in', [c.id for c in self.env.user.pos_config_ids])]
        return super().search_read(domain=domain, fields=fields, offset=offset, limit=limit, order=order)

    @api.depends('session_ids', 'session_ids.state')
    def _compute_current_session(self):
        # todo: get only one session of current user
        for pos_config in self:
            opened_sessions = pos_config.session_ids.filtered(lambda s: not s.state == 'closed' and s.user_id and s.user_id.id == self.env.user.id)
            session = pos_config.session_ids.filtered(lambda s: not s.state == 'closed' and s.user_id and s.user_id.id == self.env.user.id)
