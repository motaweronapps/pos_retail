...
class PosConfig(models.Model):
    ...
    disable_preview_kitchen_bill = fields.Boolean(
        'Disable Preview Kitchen Bill',
        help='If checked, preview Kitchen Bill will disable'
    )
    auto_print_kitchen_bill = fields.Boolean(
        'Auto Print Kitchen Bill without Popup',
        help='If check, when click to Order Kitchen button, automatic print without popup'
    )
    ...
    def onchange_sync_multi_session(self):
        if not self.sync_multi_session:
            ...
            self.send_order_to_kitchen = False
            self.allow_lock_table = False
            self.point_print = False
            self.disable_preview_kitchen_bill = False

    @api.onchange('disable_preview_kitchen_bill')
    def onchange_disable_preview_kitchen_bill(self):
        if self.disable_preview_kitchen_bill:
            self.auto_print_kitchen_bill = False
        else:
            self.auto_print_kitchen_bill = True

    @api.onchange('auto_print_kitchen_bill')
    def onchange_auto_print_kitchen_bill(self):
        if self.auto_print_kitchen_bill:
            self.disable_preview_kitchen_bill = False
        else:
            self.disable_preview_kitchen_bill = True
